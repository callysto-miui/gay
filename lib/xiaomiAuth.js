...
/**
 * Same as parseXiaomiJson, but on failure throws an XiaomiLoginError that
 * names the failing step, the HTTP status/URL of the last hop, and a
 * snippet of the raw body — instead of a bare "Unexpected token" SyntaxError
 * that gives no clue which request actually failed.
 */
function parseXiaomiJsonOrThrow(label, body, chain) {
  try {
    return parseXiaomiJson(body);
  } catch (e) {
    const last = chain[chain.length - 1];
    const snippet = String(body).slice(0, 200).replace(/\s+/g, ' ').trim();
    throw new XiaomiLoginError(
      `Step "${label}" got a non-JSON response (HTTP ${last.status} from ${last.url}). ` +
        `Raw body: "${snippet}". This is almost always Xiaomi's server blocking/rejecting ` +
        `the request (wrong region, rate-limited/blocked IP, or a bad redirect) rather than ` +
        `a bug in the cookie handling.`
    );
  }
}
...
class XiaomiAuthClient {
  ...
  async _getJson(label, url, params) {
    const { body, chain } = await this._followed('get', url, { params });
    return parseXiaomiJsonOrThrow(label, body, chain);
  }

  async _postFormJson(label, url, data) {
    const bodyStr = new URLSearchParams(data).toString();
    const { body, chain } = await this._followed('post', url, { data: bodyStr, isForm: true });
    return parseXiaomiJsonOrThrow(label, body, chain);
  }

  async _postEmptyJson(label, url) {
  ...
  async login(user, password) {
    ...
    const res1 = await this._getJson('serviceLogin', `${BASE_URL}/pass/serviceLogin`, {
      sid: SID,
      _json: 'true',
    });
    ...
    const res2 = await this._postFormJson('serviceLoginAuth2', `${BASE_URL}/pass/serviceLoginAuth2`, data);
    ...
    if (res2.notificationUrl) {
      ...
      const emailRes = await this._getJson('verifyEmail(GET)', `${BASE_URL}/identity/auth/verifyEmail`, {
        _json: 'true',
      });
      const maskedEmail = emailRes.maskedEmail || '?';

      const quotaRes = await this._postFormJson('sms/userQuota', `${BASE_URL}/identity/pass/sms/userQuota`, {
        addressType: 'EM',
        contentType: '160040',
      });
  ...
  async sendEmailCode() {
    const res = await this._postEmptyJson('sendEmailTicket', `${BASE_URL}/identity/auth/sendEmailTicket`);
  ...
  async verifyEmailCode(ticket) {
    const vRes = await this._postFormJson('verifyEmail(POST)', `${BASE_URL}/identity/auth/verifyEmail`, {
      ticket,
      trust: 'true',
    });
    ...
    const res3 = await this._getJson('serviceLogin(post-otp)', `${BASE_URL}/pass/serviceLogin`, {
      _json: 'true',
      sid: SID,
    });
  ...
  async _finishLogin(res) {
    const regionRes = await this._getJson('login/region', `${BASE_URL}/pass/user/login/region`);
